    ...
    extends="coldbox.system.testing.BaseModelTest"
{
    ...
    function beforeAll() {
        super.beforeAll();
        // setup
		model = createMock( "com.madishetti.JedisManager");
		model.$("loadSettings");
    ...
    function run() {
        describe( "JedisManager suite", function() {
			...
			it( "set and get complex value", function() {
                // Arrange
				var key   = "testKey";
				var value = [1,2,3,4,5];
				model.cacheInsert(key, value,10);
				...
				// Assert
				assertEquals(value, retrievedValue, "getValue should return the expected value");
				expect(retrievedValue).toBeArray();
            } );

			it( "set component object should throw exception", function() {
                // Arrange
				var key   = "testKey";
				var value = model;
				// Expect cacheInsert to throw com.madishetti.JedisManager.CacheInsertTypeException exception
                expect( function() {
                    model.cacheInsert(key, value,10);
                } ).toThrow( "com.madishetti.JedisManager.CacheInsertTypeException" );
            } );
